...
GetState()
{
	...
	if(1)
	{
		...
		Eigen::Vector3d out_goal;
		Eigen::Vector3d pos_diff = this->target_pos - p_com;
		pos_diff[1] = 0;
		...
		if(out_goal.norm()<0.00001){
			out_goal= {0,0,1.0};
		}
		else{
			out_goal = out_goal.normalized();	
		}
