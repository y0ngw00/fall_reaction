...
void Character::LoadSkelMap()
{
	TiXmlDocument doc;
	if(!doc.LoadFile(mPath)){
		std::cout << "Can't open Character XML file from : " << mPath << std::endl;
		return;
	}
	...
	for(TiXmlElement *body = skeldoc->FirstChildElement("Joint"); body != nullptr; body = body->NextSiblingElement("Joint"))
	{
		...
		if(body->Attribute("bvh")!=nullptr)
			mSkelMap.insert(std::make_pair(name,body->Attribute("bvh"))); //pairing xml joint name with bvh joint name
	}
}
