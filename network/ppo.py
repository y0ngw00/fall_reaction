...
class PPO(object):
	def __init__(self, learning_rate_actor=1e-5, learning_rate_critic=0.001, learning_rate_decay=0.9993,
	...
	def initTrain(self, name, env, pretrain="", directory=None, 
		batch_size=128, steps_per_iteration=2048):
		...
		if self.pretrain is not "":
			...
			if len(li) == 2:
				...
				self.env.RMS_disc.load(li[0]+'disc-rms'+suffix)
			...
			self.env_disc.RMS.setNumStates(self.num_feature)
...
if __name__=="__main__":
	...
	parser.add_argument("--ntimesteps", type=int, default=100000000)
